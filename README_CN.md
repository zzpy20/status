...
本仓库内部模块结构、以及和 `Shenzhen-Reality` 之间的关系图，见 [ARCHITECTURE.md](ARCHITECTURE.md)（英文）。
...
- **单个监控项的详情页**（`/monitor/:id`）——当前状态以及维持这个状态多久了、24 小时/7 天/30 天/365 天的 uptime 百分比（附带每个时间窗口的故障次数和总停机时长）、响应时间的平均值/最小值/最大值加一个简单的图表，以及最近（24 小时内）的故障记录表。跟状态页一样，公开、只读。
...
- **管理界面**（`/admin`）——添加和编辑监控项都通过弹窗完成（不是在行内展开），可以暂停/恢复、重置、删除、搜索，还能发一条测试通知。用一个 bearer token 保护。
- **批量操作**——通过复选框（或"全选"）选中多个监控项，一次性批量增加/删除标签、暂停、重置或删除。复用的是已有的单个监控项管理接口，在前端循环调用，而不是单独做一套批量 API——以这个工具实际的使用规模来说完全够用。重置/删除/暂停这三个操作都会先弹出确认框，说明会影响到几个监控项、后果是什么。
...
- **所有 HTML 响应都带 `Cache-Control: no-store`。** 真实踩过两次"页面缓存住了"的坑——明明通过 `curl` 确认服务端已经是修好之后的版本了，但浏览器/移动端 Safari 还是在显示旧的缓存版本，看起来就像是修复没生效。对一个流量很小的个人管理工具来说，干脆完全禁用缓存没什么代价。
- **批量操作的勾选状态不会跨重新渲染保留。** 在搜索框里打字，或者完成一次批量操作之后，整个列表都会重新渲染，勾选框会全部清空——这是刻意为之的简化处理，不是 bug，毕竟这个工具实际的使用规模也就几个到几十个监控项。
