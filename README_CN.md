...
- **单个监控项的详情页**（`/monitor/:id`）——当前状态以及维持这个状态多久了、24 小时/7 天/30 天/365 天的 uptime 百分比（附带每个时间窗口的故障次数和总停机时长，另外还单独做了一张"总停机时长（365 天）"卡片，有停机就标红、完全没有就标绿，不会被埋没在小字里）、最近 30 天响应时间的平均值/最小值/最大值加一个简单的图表，以及最近 365 天内的完整故障记录表。跟状态页一样，公开、只读。
...
- **故障判定防抖**（2026-07-19 加入）——单次检查失败（比如一次瞬时的网络抖动，或者一波扫描/探测流量短时间内打满了某台小 VPS 的代理端口——这次部署里就在一个监控项上真实遇到过）不会再单独算作一次故障、也不会触发通知；必须连续两次检查都失败，才会确认这个监控项真的挂了，而且故障记录的起始时间会回溯到第一次失败的那一刻，这样时长才准确。恢复判定是不对称的、而且是立即生效的：只要有一次检查成功，故障就立刻结束——毕竟"这个目标又能连上了"这件事本身没什么模糊地带。这一个改动影响到故障列表、所有显示当前状态的地方、状态页上的"距离上次故障"时间，以及推送通知——现在全部都基于同一套"已确认状态"的逻辑（`db.confirmedIsUp()`）来算，而不是只看最近一次原始检查结果。
