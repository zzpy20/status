...
- **Per-monitor detail page** (`/monitor/:id`) -- current status and how long it's been in that state, 24h/7d/30d/365d uptime % with incident counts and total downtime per window (plus a standalone "Total downtime (365d)" card, tinted red if there was any and green if there wasn't, so it isn't buried in fine print), response-time average/min/max over the last 30 days plus a simple chart, and a table of every incident in the last 365 days. Public/read-only, same as the status page.
...
- **Shenzhen-Reality DNS drift auto-fix** (`src/dns-drift-sync.js`, added 2026-07-11) -- a bolt-on, project-specific exception to this tool's otherwise-generic design. Both of [[project-shenzhen-reality]]'s Aliyun ECS boxes are spot instances that get a new random public IP on reclaim/restart; this runs on every cron tick, checks each instance's actual IP (via a from-scratch Aliyun RPC-signed API call, `src/aliyun.js`) against its Cloudflare DNS record (`src/cloudflare-dns.js`), and PATCHes it on drift with a Telegram alert. Lives here (not its own Worker) only because the Cloudflare account is already at the 5-cron-trigger free-plan limit -- see `Shenzhen-Reality/README.md`'s "Automated DNS drift fix" section for the full story, including a real `wrangler secret put` interactive-prompt bug hit while wiring up the required secrets (`ALIYUN_ACCESS_KEY_ID`/`SECRET`, `CF_API_TOKEN`). If this Worker is ever reused for a different project (its original design intent), this module and its 4 extra secrets/vars are the one piece that doesn't generalize -- safe to delete along with its `scheduled()` call in `index.js`.
- **Debounced down-state detection** (2026-07-19) -- a single failed check (a transient network blip, or a burst of scanner/probe traffic briefly saturating a small VPS's proxy port -- both observed for real on one monitor in this deployment) no longer counts as an incident or triggers a notification by itself; two consecutive failed checks are required to confirm a target is actually down, with the incident's recorded start backdated to that first failure so its duration stays accurate. Recovery is asymmetric and immediate: one successful check ends the incident right away, since there's no ambiguity about a target being reachable again. This one change touches the incidents list, every place the current-status badge is shown, the "last down" timestamp on the status page, and push notifications -- all now derived from the same confirmed-state logic (`db.confirmedIsUp()`) instead of the last raw check result.
